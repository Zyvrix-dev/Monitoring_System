  ...
  return (
    <aside className="metric-details" aria-live="polite">
      <header className="metric-details__header">
        ...
        <button
          type="button"
          className="metric-details__close"
          onClick={onClose}
      ...
      {description && (
        <p className="metric-details__description">{description}</p>
      )}
      ...
      {items.length > 0 && (
        <section className="metric-details__items" aria-label="Metric entries">
            ...
            <div className="metric-details__item">
              {activeItem ? (
                <>
                  ...
                  {activeItem.subtitle && (
                    <p className="metric-details__item-subtitle">
                      {activeItem.subtitle}
                    </p>
                  )}
                  {activeItem.description && (
                    <p className="metric-details__item-description">
                      {activeItem.description}
                    </p>
                  )}
                </>
              ) : (
                <p className="metric-details__item-empty">
          ...
          {items.length > 1 && (
            <div
              className="metric-details__dots"
              role="tablist"
              aria-label="Select metric entry"
            >
              {items.map((item, index) => (
                <button
                  key={item.id || item.title || index}
                  type="button"
                  className={`metric-details__dot ${
                    index === activeIndex ? "metric-details__dot--active" : ""
                  }`}
